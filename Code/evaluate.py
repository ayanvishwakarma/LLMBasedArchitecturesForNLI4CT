...
def task1_metrics(targets, predictions, uuids, args):
    ...
    metrics_dict['Task1-AUROC'] = roc_auc_score(true_entail, pred_entail)
    metrics_dict['Task1-AUPRC'] = average_precision_score(true_entail, pred_entail)
...
def task2_metrics(targets, predictions, uuids, args):
    ...
    for uuid in uuids:
        ...
        logits.extend(predictions[uuid]['Primary_evidence_prob'])
        labels.extend([1 if x in targets[uuid]['Primary_evidence_index'] else 0 for x in range(len(predictions[uuid]['Primary_evidence_prob']))])
        if targets[uuid]['Type'] == 'Comparison':
            ...
            logits.extend(predictions[uuid]['Secondary_evidence_prob'])
            labels.extend([1 if x in targets[uuid]['Secondary_evidence_index'] else 0 for x in range(len(predictions[uuid]['Secondary_evidence_prob']))])
    ...
    metrics_dict['Task2-AUROC'] = roc_auc_score(labels, logits)
    metrics_dict['Task2-AUPRC'] = average_precision_score(labels, logits)
...
def task1_perturbed_metrics(targets, predictions, uuids, args):
    ...
    consistency = sum([predictions[uuid]['Prediction'] == targets[uuid]['Label'] for uuid in preserving_uuids]) / len(preserving_uuids)
    faithfulness = sum([predictions[uuid]['Prediction'] != targets[targets[uuid]['Causal_type'][1]]['Label'] for uuid in altering_uuids]) / len(altering_uuids)
